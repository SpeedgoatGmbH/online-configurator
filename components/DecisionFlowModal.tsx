...
import type { SystemSummary } from './configurator/decisionFlowExamples'
...
const OVERVIEW_STAGES: OverviewStage[] = [
  ...
  },
  {
    ...
    description: 'Apply hard fit gate checks (board/plugin/extension/resource) and verify machine slot limits before finalizing.',
    color: 'bg-red-50',
  ...
  },
  {
    ...
    description: 'Get a complete bill-of-materials with rationale, fit diagnostics, and FPGA resource planning headroom.',
    color: 'bg-violet-50',
...
type FlowNodeData = {
  ...
  isActive?: boolean
  isDimmed?: boolean
  /** Populated only on the system_summary node when an example is active */
...
/** Terminal node: structured BOM summary rendered as a stat grid */
function SummaryNode({ data }: { data: FlowNodeData }) {
  const s = data.systemSummary

  function Stat({ label, value, accent }: { label: string; value: string | number; accent: string }) {
    return (
      <div className={`rounded-lg border px-3 py-2 text-center ${accent}`}>
        ...
        {s ? (
          <>
            <div className="grid grid-cols-2 gap-1.5">
              <Stat label="Rows resolved" value={`${s.resolvedRows} / ${s.totalRows}`}
                accent="border-emerald-200 bg-emerald-50 text-emerald-800" />
              <Stat label="Channels covered" value={s.coveredChannels}
                accent="border-blue-200 bg-blue-50 text-blue-800" />
              <Stat label="Main boards" value={s.mainBoardsQty}
                accent="border-violet-200 bg-violet-50 text-violet-800" />
              <Stat label="Interface boards" value={s.interfaceBoardsQty}
                accent="border-teal-200 bg-teal-50 text-teal-800" />
              {s.fpgaBoardsQty > 0 && (
                <Stat label="FPGA" value={s.fpgaBoardsQty}
                  accent="border-green-200 bg-green-50 text-green-800" />
              )}
              {s.dedicatedBoardsQty > 0 && (
                <Stat label="Dedicated I/O" value={s.dedicatedBoardsQty}
            ...
            {s.unresolvedCount > 0 && (
              <div className="mt-2 flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5">
                ...
                <span className="text-[10px] font-semibold text-red-700">
                  {s.unresolvedCount} row{s.unresolvedCount > 1 ? 's' : ''} unresolved — no matching module found
                </span>
              </div>
            )}
          </>
          ...
          <p className="text-[10px] text-slate-400 italic">
            Run a configuration to see system totals.
          </p>
        )}
      </div>
      <Handle type="source" position={Position.Bottom} className="!bg-slate-400" />
...
const nodeTypes = { step: StepNode, decision: DecisionNode, result: ResultNode, summary: SummaryNode }
...
const MAIN_PIPELINE_ORDER = [
  ...
  'slot_check', 'compat_check', 'output', 'system_summary',
]
...
const BASE_NODES: Node<FlowNodeData>[] = [
  ...
  },
  {
    ...
    data: { label: 'Score Candidates', description: 'Each candidate is scored against the requirement specs. Multi-criteria weighted scoring with consolidation, FPGA look-ahead, lifecycle, config-package selection, FPGA category, dedicated-simplicity bonuses, and profile-specific interface-overhead penalties.', technicalDetail: 'profile=balanced|min_modules|prefer_fpga\\nscore = exact + compat − mismatch − missing − units\\n+ consolidation + fpgaLookAhead + machine + lifecycle\\n+ configPkg(selected) + fpgaCategory + dedicatedSimplicity\\n− interfaceOverhead (profile weighted)', icon: '⚡', color: '#FDF2F8', border: '#EC4899' },
  },
  {
    id: 'pick_best', type: 'decision', position: { x: 60, y: 510 },
    data: { label: 'Pick Best or Unresolved', description: 'Candidates sorted by profile-specific composite key after hard fit gate filtering. If no candidates pass, the row becomes "unresolved" with a suggestion.', technicalDetail: 'fit gate first: board/plugin/extension/resource feasibility\nbalanced: min(units) → max(score) → min(channelCapacity) → seed → moduleId\nmin_modules: min(totalItems) → min(units) → max(score)\nprefer_fpga: max(score) → min(totalItems) → min(units)', icon: '🏆', color: '#FFF7ED', border: '#F97316' },
  },
  {
  ...
  },
  {
    ...
    data: { label: 'Consolidate FPGA Boards', description: 'Functions sharing the same fpgaFamily merge onto fewer physical boards. Dual budget: signal capacity (from fit model) AND FPGA resource utilization (slices, LUTs, registers, RAM, DSP).', technicalDetail: 'boardsByCapacity = ⌈max(qty/signalCap)⌉\nboardsByResources = ⌈Σ(codeModule.base + perCh×qty) / board.resources⌉\nactual = max(boardsByCapacity, boardsByResources)', icon: '🧩', color: '#ECFDF5', border: '#10B981' },
  },
  {
  ...
  },
  {
    ...
    data: { label: 'Proposal Output', description: 'Final proposal: recommended modules with quantity and rationale. Includes per-row diffs, unresolved items, machine warnings, fit diagnostics, and FPGA resource planning summary.', technicalDetail: 'Aggregate per-module rationale from spec alignment\n+ consolidation notes + machine fit\n+ fit diagnostics (hard gate + fail-open counts)\n+ FPGA resource planning headroom (slices/lut/register/ram16/ram8/dsp)', icon: '📋', color: '#DBEAFE', border: '#2563EB' },
  },
  {
    id: 'system_summary', type: 'summary', position: { x: 60, y: 1170 },
    data: { label: 'System Overview', description: 'BOM at a glance: boards, channels, and coverage across the full system.', icon: '🗂', color: '#FAF5FF', border: '#9333EA' },
  },
]
...
const EDGES: Edge[] = [
  ...
  { id: 'e-out-summary', source: 'output', target: 'system_summary', style: { ...EDGE_STYLE, strokeDasharray: '4 3' }, markerEnd: MARKER },
...
export default function DecisionFlowModal({ open, onClose, liveExample }: DecisionFlowModalProps) {
  ...
  const computedNodes: Node<FlowNodeData>[] = useMemo(() => {
    ...
    return BASE_NODES.map((node) => {
      ...
      const extraData = node.id === 'system_summary' && isVisited && activeExample.systemSummary
        ? { systemSummary: activeExample.systemSummary }
        : {}

      return {
        ...node,
        data: { ...node.data, isActive: isCurrentlyActive, isDimmed: shouldDim, exampleFact: fact, ...extraData },
      }
    })
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {activeExample?.optimizationProfile && (
                <p className="text-[10px] font-semibold uppercase tracking-wide text-violet-600">
                  Active profile: {activeExample.optimizationProfile}
                </p>
              )}
            </div>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {isFpgaStage && activeExample?.fpgaConsolidation && isVisited && (() => {
                            ...
                            const withInterfaces = after + ifcCount
                            return (
                              <div className="mt-3 flex items-stretch gap-3">
                                <div className="flex-1 rounded-lg border border-red-200 bg-red-50 p-3 text-center">
                                  ...
                                  <p className="text-xs text-red-600">FPGA board{before !== 1 ? 's' : ''} · {before} base slot{before !== 1 ? 's' : ''}</p>
                                </div>
                                ...
                                <div className="flex-1 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-center">
                                  ...
                                  <p className="text-xs text-emerald-600">{after} FPGA board{after !== 1 ? 's' : ''} · {after} base slot{after !== 1 ? 's' : ''}</p>
                                  {ifcCount > 0 && (
                                    <p className="text-[10px] text-emerald-500 mt-0.5">+ {ifcCount} interface board{ifcCount !== 1 ? 's' : ''} ({withInterfaces} total slot{withInterfaces !== 1 ? 's' : ''} incl. interfaces)</p>
                                  )}
                                </div>
                                {saved > 0 && (
                                  <div className="flex items-center">
                                    <div className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-700 ring-1 ring-emerald-300">
                                      {saved} FPGA board{saved !== 1 ? 's' : ''} saved
                                    </div>
                                  </div>
                                )}
                              </div>
