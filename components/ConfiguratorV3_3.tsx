...
function getProtocolFamilyLabel(protocol: string): string {
  ...
  if (protocol === 'MIL-STD-1553') return 'MIL-STD'

  if (protocol === 'CAN' || protocol === 'CAN FD' || protocol === 'CANopen' || protocol.includes('XCP over CAN')) {
    return 'CAN Family'
  }
  ...
  if (
    ...
    protocol === 'Real-Time UDP'
  ) {
    return 'Ethernet Family'
  }

  if (protocol.startsWith('Modbus')) return 'Modbus Family'
  if (protocol === 'PROFIBUS' || protocol === 'DNP3' || protocol === 'IEC 61850' || protocol === 'MVB / WTB') return 'Industrial Buses'
  if (protocol === 'LIN' || protocol === 'FlexRay' || protocol === 'SENT' || protocol === 'PSI5') return 'Automotive Buses'
  if (protocol.startsWith('RS-') || protocol === 'SPI' || protocol === 'I2C') return 'Serial / Peripheral'
  if (protocol.includes('PTP') || protocol.includes('IRIG')) return 'Timing / Sync'
  if (protocol === 'MQTT' || protocol === 'DDS') return 'Middleware'
  if (protocol === 'Aurora' || protocol === 'Shared Memory') return 'High-Speed Links'
...
function buildProtocolFamilyGroupsLocal(protocols: string[]): ProtocolFamilyGroup[] {
  ...
  protocols.forEach((protocol) => {
    const family = getProtocolFamilyLabel(protocol)
    if (!familyMap[family]) {
      familyMap[family] = []
      familyOrder.push(family)
    }
    familyMap[family].push(protocol)
  })

  return familyOrder.map((family) => ({
    label: family,
    protocols: familyMap[family],
  }))
}
...
export default function ConfiguratorV3({ onSummaryChange, onRequirementsChange }: ConfiguratorHookProps = {}) {
  ...
  const renderProtocolSelectorOverlay = () => {
    ...
    const isFamilyOpen = (industry: ProtocolIndustry, familyLabel: string, familyProtocols: string[]) => {
      const familyKey = `${industry}:${familyLabel}`
      if (openProtocolFamilies[familyKey] !== undefined) {
        return openProtocolFamilies[familyKey]
      }
      return Boolean(selectedProtocol && familyProtocols.includes(selectedProtocol))
    }
      ...
      <div className="fixed inset-0 z-[110]">
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center p-3">
          <div className="pointer-events-auto w-full max-w-[860px]">
            <div className="flex flex-col rounded-[var(--ui-radius-lg)] border border-slate-200 bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-2 border-b border-slate-200 px-3 py-1.5">
              ...
              <div className="space-y-1.5 px-3 py-2">
                <div className="grid grid-cols-[92px_minmax(0,1fr)] rounded-[var(--ui-radius-sm)] border border-slate-200 bg-slate-50 px-2 py-1">
                ...
                <div className="rounded-[var(--ui-radius-sm)] border border-slate-200 bg-white">
                  {industryGroups.map((group, index) => (
                    <div
                      key={group.industry}
                      className={cn('grid grid-cols-[92px_minmax(0,1fr)]', index > 0 && 'border-t border-slate-200')}
                    >
                      <div className="bg-slate-50 px-2 py-1.5 text-xs font-semibold text-slate-700">{group.industry}</div>
                        ...
                        {group.protocols.length > 0 ? (
                          <div className="space-y-1">
                            {buildProtocolFamilyGroupsLocal(group.protocols).map((family) => {
                              ...
                              return (
                                <div key={familyKey} className="rounded-[var(--ui-radius-sm)] border border-slate-200">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      setOpenProtocolFamilies((prev) => ({
                                        ...prev,
                                        [familyKey]: !open,
                                      }))
                                    }
                                    className="flex h-7 w-full items-center justify-between bg-slate-50 px-2 text-left text-xs font-semibold text-slate-700"
                                    aria-expanded={open}
                                    aria-controls={`family-${familyKey}`}
                                  >
                                    <span className="truncate pr-2">{family.label}</span>
                                    <span className="text-[11px] text-slate-500">{open ? 'Hide' : 'Show'} · {family.protocols.length}</span>
                                  </button>

                                  {open && (
                                    <div id={`family-${familyKey}`} className="grid grid-cols-1">
                                      {family.protocols.map((protocol, protocolIndex) => {
                                        ...
                                        return (
                                          <button
                                            ...
                                            className={cn(
                                              'flex h-7 w-full items-center justify-between px-2 text-left text-xs transition',
                                              protocolIndex > 0 && 'border-t border-slate-200',
                                              isSelected
                                                ? 'bg-[rgb(var(--speedgoat-blue))]/10 text-[rgb(var(--speedgoat-blue))]'
                                                : 'bg-white text-slate-700 hover:bg-slate-50'
                                            )}
                                            aria-label={`${group.industry} ${protocol}`}
                                          >
                                            <span className="truncate pr-2">{protocol}</span>
                                            <span
                                              className={cn(
              ...
              <div className="flex items-center justify-end gap-[var(--ui-gap-1)] border-t border-slate-200 px-3 py-2">
                <CompactButton type="button" variant="secondary" onClick={closeProtocolSelector}>
                  Cancel
                </CompactButton>
              </div>
            </div>
