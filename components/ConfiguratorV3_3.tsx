...
const ADDITIONAL_DISPLAY_GROUPS: [DisplayGroup, DisplayGroup | null][] = (() => {
  // Collect categories that have at least one unclaimed sub
...
export default function ConfiguratorV3({ onSummaryChange, onRequirementsChange, onLoadTemplate, onSignalRowsChange, closedLoopRate = '10k' }: ConfiguratorV3Props = {}) {
  const {
    signalRows,
    setSignalRows,
    editingTarget,
  ...
  // Collect tier2 rows that have been actively configured (qty > 0 or non-base)
  const activeTier2Rows = useMemo(() => {
    const rows: { categoryId: string; sub: SubCategory; row: SignalRow; index: number; categoryLabel: string }[] = []
    const tier2All = tier2Categories.concat(tier1Categories).filter((cat) =>
      cat.subCategories.some((sub) => !DISPLAY_GROUP_CLAIMED.has(`${cat.id}:${sub.id}`))
    )
    tier2All.forEach((cat) => {
      cat.subCategories.forEach((sub) => {
        if (DISPLAY_GROUP_CLAIMED.has(`${cat.id}:${sub.id}`)) return
        const subRows = signalRows[cat.id]?.[sub.id] || []
        subRows.forEach((row, idx) => {
          // Show rows that either have quantity or are non-base (user added them)
          if (row.quantity > 0 || !row.id.endsWith('-base')) {
            rows.push({ categoryId: cat.id, sub, row, index: idx, categoryLabel: cat.label })
          }
        })
      })
    })
    return rows
  }, [signalRows, tier2Categories, tier1Categories])
  ...
  const renderInlineSpecSelect = (
    ...
    if (categoryId === 'communication' && sub.id === 'protocols' && fieldKey === 'range') {
      return (
        <button
          ...
          className="inline-flex h-6 cursor-pointer items-center rounded-[var(--ui-radius-sm)] border border-slate-200 bg-white px-1.5 text-[11px] font-semibold text-slate-700 transition hover:border-[rgb(var(--speedgoat-blue))] hover:bg-[rgb(var(--speedgoat-blue))]/5 hover:text-[rgb(var(--speedgoat-blue))]"
          title={fieldLabel}
        >
    ...
    const fieldDef = sub.fields.find((f) => f.key === fieldKey)
    const hasTooltip = fieldDef?.tooltip

    return (
      <span key={fieldKey} className="inline-flex min-w-[78px] max-w-full items-center gap-0.5">
        <span className="relative inline-flex min-w-0 flex-1 items-center">
          <select
            ...
            className="h-6 w-full cursor-pointer appearance-none rounded-[var(--ui-radius-sm)] border border-slate-200 bg-white py-0 pl-1.5 pr-4 text-[11px] font-medium text-slate-700 transition hover:border-[rgb(var(--speedgoat-blue))] hover:bg-[rgb(var(--speedgoat-blue))]/5 hover:text-[rgb(var(--speedgoat-blue))] focus:outline-none"
          ...
          <span className="pointer-events-none absolute right-1 text-[8px] text-slate-400">▾</span>
        </span>
        {hasTooltip && (
          <CompactTooltip
            className="ml-0 shrink-0 [&_svg]:h-3 [&_svg]:w-3"
            content={fieldDef.tooltip}
          />
        )}
      </span>
    )
  ...
  /* ── Signal row: Label | [qty] ▼ | spec₁ ▼ | spec₂ ▼ | … | [x] ── */
  const renderSignalRow = (categoryId: string, sub: SubCategory, row: SignalRow, rowIndex: number) => {
    ...
    return (
      <div
        key={row.id}
        className={cn(
          'group flex flex-wrap items-center gap-1.5 rounded-[var(--ui-radius-sm)] border px-2 py-1.5 transition-all duration-150 sm:flex-nowrap',
          hasQuantity
            ? 'border-slate-200 bg-white shadow-[0_1px_0_rgba(15,23,42,0.05)] hover:border-slate-300 hover:shadow-sm'
            : 'border-dashed border-slate-200 bg-slate-50/60'
        )}
      >
        {/* Sub-category label */}
        <span className="min-w-[68px] shrink-0 text-[11px] font-semibold leading-5 text-slate-700 sm:min-w-[80px]">
          {sub.label}
        </span>
        ...
        <div className="relative shrink-0">
          <select
            ...
            className={cn(
              'h-6 w-[48px] cursor-pointer appearance-none rounded-[var(--ui-radius-sm)] border bg-white pl-1 pr-3.5 text-right text-[11px] font-semibold tabular-nums transition focus:outline-none',
              hasQuantity
          ...
          <span className="pointer-events-none absolute right-1 top-1/2 -translate-y-1/2 text-[8px] text-slate-400">▾</span>
        </div>
        <span className="shrink-0 text-[10px] font-medium uppercase tracking-wide text-slate-400">ch</span>

        {/* Inline spec dropdowns — only shown when quantity > 0 */}
        {hasQuantity && (
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-0.5 sm:flex-nowrap">
        ...
        {/* Spacer when no specs shown */}
        {!hasQuantity && <div className="min-w-0 flex-1" />}

        {/* Remove button (non-base rows) */}
        {!isBaseRow ? (
          <button
            type="button"
            onClick={() => removeSignalRow(categoryId, sub.id, row.id)}
            className="flex h-4 w-4 shrink-0 items-center justify-center rounded text-[9px] font-bold text-slate-400 opacity-0 transition hover:bg-red-50 hover:text-red-500 group-hover:opacity-100"
  ...
  const renderCategoryBlock = (category: (typeof CATEGORIES)[number]) => {
    ...
    return (
      <div key={category.id} className="py-2 first:pt-0">
        ...
        <div className="mb-2 flex items-center justify-between px-1">
          ...
          {totalSignals > 0 && (
            <span className="inline-flex min-w-[24px] justify-center rounded-full bg-slate-100 px-1.5 py-px text-[10px] font-semibold tabular-nums text-slate-600">
              {totalSignals}
            </span>
          )}
        </div>
        ...
        <div className="mt-1.5 px-0.5">
          {addingForCategory === category.id && hasManySubcategories ? (
            <div className="flex items-center gap-1">
              ...
              {category.subCategories.map((sub) => (
                <button
                  ...
                  className="inline-flex h-6 items-center rounded-[var(--ui-radius-sm)] border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-700 transition hover:border-[rgb(var(--speedgoat-blue))] hover:bg-[rgb(var(--speedgoat-blue))]/5 hover:text-[rgb(var(--speedgoat-blue))]"
                >
                  {sub.label}
                </button>
              ))}
            ...
            <CompactAddLink
              ...
              className="mt-0.5 h-6 px-1 text-xs text-slate-600 hover:text-[rgb(var(--speedgoat-blue))]"
            >
    ...
    return (
      <div key={group.id} className="py-1 first:pt-0">
        ...
        <button
          ...
          className="mb-0.5 flex w-full items-center justify-between px-1"
        >
          <span className="flex items-center gap-1">
            <svg
              className={cn('h-2.5 w-2.5 text-slate-400 transition-transform', !isCollapsed && 'rotate-90')}
        ...
        {!isCollapsed && (
          <>
            ...
            {/* Add link with inline subcategory picker */}
  ...
  return (
    <>
      <CompactCard
        variant="default"
        className="overflow-hidden border-slate-200 bg-gradient-to-b from-white to-slate-50/50 p-3 shadow-[0_8px_24px_rgba(15,23,42,0.04)]"
      >
        {/* All display groups — core + additional, single 2-column flow */}
        <div className="space-y-1 divide-y divide-slate-200/90">
          {DISPLAY_GROUPS.map(([left, right], rowIdx) => (
            <div key={rowIdx} className="grid grid-cols-1 items-start gap-x-3 min-[640px]:grid-cols-2">
              {renderDisplayGroup(left)}
              {renderDisplayGroup(right)}
            </div>
          ))}
          {ADDITIONAL_DISPLAY_GROUPS.map(([left, right], rowIdx) => (
            <div key={`add-${rowIdx}`} className="grid grid-cols-1 items-start gap-x-3 min-[640px]:grid-cols-2">
              {renderDisplayGroup(left)}
