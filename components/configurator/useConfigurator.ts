...
export type SpecSummaryEntry = { key: FieldKey; value: string }
  ...
  CATEGORIES.forEach((category) => {
    ...
    category.subCategories.forEach((sub) => {
      initial[category.id][sub.id] = [
        {
          id: `${category.id}-${sub.id}-base`,
          quantity: 0,
          specs: normalizeSpecsForSub(sub, { ...sub.defaults }),
        },
      ]
    })
  })
...
export function getSpecSummaryTokens(
  ...
  return getSpecSummaryEntries(sub, specs, context).map((entry) => entry.value)
}

export function getSpecSummaryEntries(
  sub: SubCategory,
  specs: SpecsRecord,
  context?: { categoryId: string; subId: string }
): SpecSummaryEntry[] {
  ...
  if (isProtocolGroup) {
    ...
    ;(['range', 'resolution', 'speed'] as const).forEach((key) => {
  ...
  const preferred = sub.fields
    ...
    .map((field) => ({ key: field.key, value: specs[field.key] }))
    .filter((entry): entry is SpecSummaryEntry => Boolean(entry.value))
...
export interface UseConfiguratorReturn {
  ...
  updateRowSpec: (categoryId: string, subId: string, rowId: string, fieldKey: FieldKey, value: string) => void
  saveDraft: () => void
...
export function useConfigurator(options: UseConfiguratorOptions): UseConfiguratorReturn {
  ...
  const addSignalRow = (categoryId: string, subId: string, presetSpecs?: Partial<SpecsRecord>) => {
    ...
    const row: SignalRow = {
      ...
      quantity: sub.defaultQuantity ?? 0,
  ...
  const updateRowSpec = (categoryId: string, subId: string, rowId: string, fieldKey: FieldKey, value: string) => {
    const sub = getSubCategory(categoryId, subId)
    if (!sub) return
